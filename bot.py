...
class COLABot:
    ...
    async def process(self, req: Request):
        ...
        if not self.webex_client_signing_secret:
            logging.warning(
                "Required: You must include a clientSigningSecret to verify incoming API webhooks"
            )
        ...
        if (
            request_dict["resource"] == "memberships"
            and request_dict["event"] == "deleted"
            and request_dict["data"]["personId"] == CONFIG.BOT_ID
        ):
            return {"status_code": 200}
        if (
            request_dict["resource"] == "messages"
            and request_dict["event"] == "created"
            and request_dict["data"]["personId"] == CONFIG.BOT_ID
        ):
            return {"status_code": 200}
        ...
        with pymongo.MongoClient(mongo_url) as client:
            ...
            query_lab_filter = {
                "sender": self.activity.get("sender"),
                "roomId": self.activity.get("roomId"),
            }
            try:
                result = posts.find_one(
                    query_lab_filter
                )  # Q: Is this lab already in DB?
            ...
            if result:
                ...
                last_dialogue_epoch_time = int(
                    time.mktime(time.strptime(result["created"], pattern))
                )
                # If somehow the dialogue_max_step is maxed out
                if (
                    epoch_time_now - last_dialogue_epoch_time
                    >= int(CONFIG.DIALOGUE_TIMEOUT)
                    or result["dialogue_step"] > result["dialogue_max_steps"]
                ):  # Remove stale convos
                    try:
                ...
                # Update the activity record with the dialogue fields
                    ...
                    self.activity["dialogue_max_steps"] = result.get(
                        "dialogue_max_steps", ""
                    )
                    self.activity["dialogue_data"] = result.get("dialogue_data", "")
                    self.activity["card_dialogue_index"] = result.get(
                        "card_dialogue_index", ""
                    )
                    self.activity["card_feature_index"] = result.get(
                        "card_feature_index", ""
                    )
        ...
        elif (self.activity["description"] == "member_joined") and (
            self.activity["roomId"] == CONFIG.AUTHORIZED_ROOMS
        ):
            message = dict(
        ...
        elif (self.activity["description"] == "member_left") and (
            self.activity["roomId"] == CONFIG.AUTHORIZED_ROOMS
        ):
        ...
        # pointers to running dialogues
            ...
            if self.activity.get("text") == "help":
                ...
                self.activity.get("text") == "create accounts"
                or self.activity.get("text") == "reset passwords"
            ):
                await awx.create_accounts(self.activity)
    ...
    async def card_catch_all(self):
        if self.activity.get("description") == "card_details":
            message = dict(
                text="The card is inactive. Please generate a new one.",
                roomId=self.activity["roomId"],
                attachments=[],
            )
            await self.webex_client.post_message_to_webex(message)
    ...
    async def translate_request_to_activity(self, request_dict):
        ...
        if (
            ...
            request_dict["resource"] == "attachmentActions"
            and request_dict["event"] == "created"
        ):
            message_body = await self.webex_client.get_card_attachment(
                request_dict["data"]["id"]
            )
            activity = {
                "id": request_dict["data"]["id"],
                "resource": request_dict["resource"],
            ...
            message_body = await self.webex_client.get_message_details(
                request_dict["data"]["id"]
            )
            activity = {
                "id": request_dict["data"].get("id", ""),
    ...
    async def bot_added(self):
        text = self.generate_help_menu_markdown(help_menu_list)
        message = dict(text=text, roomId=self.activity.get("roomId"), attachments=[])
        response = await self.webex_client.post_message_to_webex(message)
        return response

    def generate_help_menu_markdown(self, help_menu):
        if (self.activity["roomType"] == "group") and (
            self.activity["roomId"] == CONFIG.AUTHORIZED_ROOMS
        ):
            markdown = "Hi, See my available commands below: \n"
            for i in help_menu:
                markdown += " - " + "**" + "@" + self.activity["bot_name"] + "** " + i
        elif (self.activity["roomType"] == "group") and (
            self.activity["roomId"] != CONFIG.AUTHORIZED_ROOMS
        ):
