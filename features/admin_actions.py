...
async def webex_not_admin(activity):
    ...
    message = dict(
        text="Sorry. I don't see your name on the COLAB administrators list '&#x1F641;'",
...
async def webex_admin_in_group_room(activity):
    webex = WebExClient(webex_bot_token=activity["webex_bot_token"])
    message = dict(
        text="Please direct message me with admin requests",
        roomId=activity["roomId"],
        attachments=[],
    )
    await webex.post_message_to_webex(message)
...
async def admin_alert_cml_users(activity):
    ...
    if activity.get("text") == "admin alert cml users":
        ...
        if cml_servers:
            ...
            message = dict(
                text="CML alert users server choices",
                roomId=activity["roomId"],
                attachments=[
                    {
    ...
    if (
        ...
        if activity.get("inputs").get("CML servers"):
            ...
            message = dict(
                text="I thought we were talking about admin alerting cml users. Let's start over.",
                roomId=activity["roomId"],
                attachments=[],
            )
            await webex.post_message_to_webex(message)
    ...
    if (
        activity.get("dialogue_name") == "cml_alert_server_choices"
        and activity.get("dialogue_step") == 3
    ):
        webex = WebExClient(webex_bot_token=activity["webex_bot_token"])
    ...
    if not await cml.get_token():
        ...
        if not await cml.get_diagnostics():
            ...
            for user in cml.diagnostics["user_list"]:
                ...
                if len(user["labs"]) and new_directory.get(user_details['username']):
                    mentions.append(f"<@personEmail:{new_directory.get(user_details['username'])}|{user_details['username']}>")  
    if mentions:
