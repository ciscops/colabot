...
class Dynamoapi:
    ...
    def get_dynamo_cml_table(self) -> None:
        """
    ...
    def get_cml_user_labs(self, user_email: str) -> dict:
      ...
      for lab_id,lab_created_date in table_labs.items():
        labs[lab_id] = datetime.strptime(lab_created_date, self.table_date_format)

      return labs

    def update_cml_lab_date(self, email: str, lab_id: str, date: datetime.date):
        """Updates the last_used date for lab"""
        date_string = datetime.strftime(date, self.table_date_format)
        self.get_dynamo_cml_table()
        try:
            self.cml_table.update_item(
                Key={'email': email},
                UpdateExpression="set #user.#email= :lab",
    ...
    def add_cml_lab(self, email: str, lab_id: str, date: datetime.date):
        #TODO: what if no labs?? need setup lab field in addition to adding
